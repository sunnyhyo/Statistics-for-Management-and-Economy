...
logit_model_4 <- glm( deny ~ black + pi_rat + hse_inc + ltv_med + ltv_high + ccred + 
                       mcred + pubrec + denpmi + selfemp + single + hischl, family=binomial(link="logit"),    
                     data=mortdeny.data)
...
OR.U <- exp(logit_model_4$coefficients + 1.96*summary(logit_model_4)$coef[,2])
OR.L <- exp(logit_model_4$coefficients - 1.96*summary(logit_model_4)$coef[,2])
